                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            ...
                            <button class="btn btn-print" type="button" data-toggle="modal"
                                data-target="#cetakModal">Export Laporan</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ url('/report/workers/export/' . $worker->id) }}" name="export_form" method="POST"
                            target="_blank">
                            @csrf
                            <div class="row">
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    <div class="form-group row">
                                        <div class="col-10 p-0 offset-col-1">
                                            <p>Pilih laporan<span class="txt-info ml-3">Pilihan dapat lebih dari 1</span>
                                            </p>
                                        </div>
                                        <div class="col-5 p-0 offset-col-1 border rounded-left">
                                            <div class="form-check form-check-flat ml-4">
                                                <label class="form-check-label">
                                                    <input type="checkbox" class="form-check-input checkbox_laporan"
                                                        data-check="0" name="laporan[]" value="pasok"> Pasok</label>
                                            </div>
                                        </div>
                                        <div class="col-5 p-0 border rounded-right">
                                            <div class="form-check form-check-flat ml-4">
                                                <label class="form-check-label">
                                                    <input type="checkbox" class="form-check-input checkbox_laporan"
                                                        data-check="0" name="laporan[]" value="transaksi"> Transaksi</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12 d-flex justify-content-start align-items-center">
                            ...
                            <div class="p-group ml-3">
                                ...
                                @elseif ($user->role == 'manager')
                                    <span class="btn manager-span">{{ $user->role }}</span>
                                @else
                    ...
                    <div class="row">
                        ...
                        <div class="col-4 d-flex justify-content-end align-items-center">
                            <div class="input-group big-search">
                                ...
                                <input type="text" class="form-control form-control-lg mr-2" name="search"
                                    placeholder="Cari data">
                            </div>
                            <div class="dropdown small-search" hidden="">
                                <button class="btn-search shadow-sm mr-2" type="button" id="dropdownMenuIconButton1"
                                        ...
                                        <table class="table table-custom">
                                            ...
                                            @foreach ($supplies as $supply)
                                                <tr>
                                                    <td class="td-1">
                                                        <span
                                                            class="d-block font-weight-bold big-font">{{ $supply->nama_barang }}</span>
                                                        <span
                                                            class="d-block mt-2 txt-light">{{ date('d M, Y', strtotime($supply->created_at)) . ' pada ' . date('H:i', strtotime($supply->created_at)) }}</span>
                                                    </td>
                                                    <td class="td-2 font-weight-bold">{{ $supply->kode_barang }}</td>
                                                    <td class="td-3 font-weight-bold"><span
                                                            class="ammount-box bg-secondary"><i
                                                                class="mdi mdi-cube-outline"></i></span>{{ $supply->jumlah }}
                                                    </td>
                                                    <td class="font-weight-bold td-4"><input type="text" name="harga"
                                                            value="{{ $supply->harga_beli }}" hidden=""><span
                                                            class="ammount-box bg-green"><i
                                                                class="mdi mdi-coin"></i></span>Rp.
                                                        {{ number_format($supply->harga_beli, 2, ',', '.') }}</td>
                                        ...
                                        <table class="table table-custom">
                                            ...
                                            @foreach ($transactions as $transaction)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button class="btn btn-selengkapnya font-weight-bold" type="button"
                                                            data-target="#dropdownTransaksi{{ $transaction->kode_transaksi }}"><i
                                                                class="mdi mdi-chevron-down arrow-view"></i></button>
                                                    </td>
                                                </tr>
                                                <tr id="dropdownTransaksi{{ $transaction->kode_transaksi }}"
                                                    data-status="0" class="dis-none">
                                                    <td colspan="5" class="dropdown-content">
                                                        ...
                                                        <table class="table">
                                                            @foreach ($products as $product)
                                                                <tr>
                                                                    <td><span
                                                                            class="numbering">{{ $loop->iteration }}</span>
                                                                    </td>
                                                                    <td>
                                                                        <span
                                                                            class="bold-td">{{ $product->nama_barang }}</span>
                                                                        <span
                                                                            class="light-td mt-1">{{ $product->kode_barang }}</span>
                                                                    </td>
                                                                    <td><span class="ammount-box-2 bg-secondary"><i
                                                                                class="mdi mdi-cube-outline"></i></span>
                                                                        {{ $product->jumlah }}</td>
                                                                    <td>
                                                                        <span class="light-td mb-1">Harga</span>
                                                                        <span class="bold-td">Rp.
                                                                            {{ number_format($product->harga, 2, ',', '.') }}</span>
                                                                    </td>
                                                                    <td>
                                                                        <span class="light-td mb-1">Total Barang</span>
                                                                        <span class="bold-td">Rp.
                                                                            {{ number_format($product->total_barang, 2, ',', '.') }}</span>
                                                                    </td>
                                                                </tr>
                                                            @endforeach
                                                        </table>
