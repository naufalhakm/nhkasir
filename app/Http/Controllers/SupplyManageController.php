...
class SupplyManageController extends Controller
{
    ...
    public function viewSupplyReturn()
    {
        $id_account = Auth::id();
        $check_access = Acces::where('user', $id_account)
        ->first();
        ...
        if($check_access->kelola_barang == 1 && $supply_system->status == true){
            ...
            return view('manage_product.supply_product.supply2', compact('dates'));
        }else{
            return back();
        }
    }
    ...
    public function viewDeleteSupply()
    {
        $id_account = Auth::id();
        $check_access = Acces::where('user', $id_account)
        ->first();
        ...
        if($check_access->kelola_barang == 1 && $supply_system->status == true){
            ...
            return view('manage_product.supply_product.return_supply', compact('products'));
        }else{
            return back();
        }
    }
    ...
    public function createSupply(Request $req)
    {
      ...
      // Update Supply
    public function updateSupply(Request $req)
    {
        $id_account = Auth::id();
        $supply_system = Supply_system::first();
        $check_access = Acces::where('user', $id_account )
        ->first();
        if($check_access->kelola_akun == 1 && $supply_system->status == true){
            $user_account = Supply::find($req->id);
