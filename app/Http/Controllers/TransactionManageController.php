...
class TransactionManageController extends Controller
{
    ...
    public function transactionProcess(Request $req)
    {
        ...
        if($check_access->transaksi == 1){
        	...
        	for($i = 0; $i < $jml_barang; $i++){
        		...
        		$transaction->total = $req->total + ($req->subtotal * $req->ppn / 100);
    ...
    public function receiptTransaction($id)
    {
        ...
        if($check_access->transaksi == 1){
            ...
            ->select('transactions.*')
            ->get();
            $ppn = $transaction->subtotal * $transaction->ppn / 100;
            $diskon = $transaction->subtotal * $transaction->diskon / 100;

            $customPaper = array(0,0,400.00,283.80);
            $pdf = PDF::loadview('transaction.receipt_transaction', compact('transaction', 'transactions', 'ppn', 'diskon', 'market'))->setPaper($customPaper, 'landscape');
            return $pdf->stream();
        }else{
            return back();
        }
